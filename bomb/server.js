...
/*
 * Сервер «Бомбы»: HTTP (создание комнаты, health, long-poll) + WebSocket (игра). bomb-spec.md §8.
 * Скелет перенесён из roulette/server.js (а туда — из auction/server.js): комнаты, токены, грация офлайна,
 * лимиты, дамп, poll-транспорт работают так же и по тем же причинам (подробности — в комментариях там).
 * Главное отличие: у сервера есть тайна — длина фитиля. Она живёт только в состоянии комнаты и в дампе
 * на диске; снимки для клиентов собирает game.snapshot, который её не отдаёт (§3).
 * Ботов нет (решение владельца): играют только те, кто зашёл с телефона.
 *
 * Запуск:  node server.js            (порт PORT, по умолчанию 3300)
...
// случайность партии — криптографическая: фитиль и первый держатель не должны угадываться
...
const ROOM_WORDS = (
  "BOMB BOOM FUSE SPARK FLASH BANG POP ZAP KAPOW WHAM TNT DYNAMO ROCKET TURBO NEON LASER PIXEL ROBOT " +
...
function createRoom({ settings = {}, speed = 1, ip = "" } = {}) {
  ...
  const room = newRoom({
    ...
    game: Game.create({ settings, code, rnd, bytes }),
...
/*
 * Снимки: доске — с испытаниями всех держателей, телефону — со своим испытанием (без ответов, без фитиля).
 * Собираем по одному разу на вид, а не на каждый сокет.
 */
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === "/bomb/api/msg" && req.method === "POST") {
  ...
  if (url.pathname === "/bomb/api/health") return json(200, { ok: true, rooms: rooms.size });
  if (url.pathname === "/bomb/api/rooms" && req.method === "POST") {
    const body = await readJson(req);
    try {
...
server.on("upgrade", (req, socket, head) => {
  const url = new URL(req.url, "http://x");
  if (url.pathname !== "/bomb/ws") return socket.destroy();
  const room = roomFor(req, url);
...
function handle(room, client, msg) {
  ...
  // Срок наступил, а таймер ещё не проснулся: сначала взрыв, потом действие.
  // Иначе бросок, пришедший после взрыва, успел бы спасти держателя (§8).
  const due = g.tick(t);
  ...
  switch (msg.type) {
    ...
    // ---- игрок ----
    case "answer": return me ? act("answer", g.answer(me, Number(msg.cid), msg.ans, t)) : undefined;
    case "pass": return me ? act("pass", g.pass(me, String(msg.to || ""), t)) : undefined;
    case "react": {
      if (!me) return;
    ...
    case "settings": {
      ...
      g.s.settings = clampSettings({ ...g.s.settings, ...(msg.settings || {}), groups: { ...g.s.settings.groups, ...((msg.settings || {}).groups || {}) } });
...
function dump() {
  try {
    // фитиль и соль пишем: без них после рестарта нечего взрывать и нечего вскрывать (§3). Файл — только на сервере.
...
/*
 * После перезапуска посреди раунда сроки не сдвигаем: фитиль запечатан, и его длина — обещание.
 * Взрыв, пропущенный за время простоя, случается сразу у того, кто держал бомбу в момент дампа (§8).
 */
function restore() {
  try {
    ...
    for (const r of JSON.parse(fs.readFileSync(DUMP, "utf8"))) {
      ...
      const room = newRoom({ code: r.code, ip: r.ip || "", hostToken: r.hostToken, game: Game.from(r.state, rnd, bytes), tokens: r.tokens || {}, touched: r.touched });
